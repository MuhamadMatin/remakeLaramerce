    ...
    <div class="flex flex-1 relative">
      ...
      <main class="flex-1 w-full min-w-0 p-4 md:p-6 lg:p-8 flex flex-col gap-6">
        ...
        <nav class="flex items-center gap-2 text-xs md:text-sm text-gray-500">
          ...
          <p class="text-gray-700 font-medium">Edit</p>
        </nav>
        ...
        <form @submit.prevent="submit">
          ...
          >
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
              ...
              <!-- Image -->
              <div class="flex flex-col gap-1.5 md:col-span-2">
                <label for="image" class="text-sm font-medium text-gray-800">
                  Image Brand
                </label>
                ...
                <div v-if="previewUrl" class="mb-2">
                  <img
                    ...
                    alt="Image Preview"
                    width="150"
                    class="rounded-lg border border-gray-200 object-cover shadow-xs"
                  />
                </div>
                ...
                <p class="mt-1 text-xs text-red-500" v-if="form.errors.image">
                  {{ form.errors.image }}
                </p>
              </div>
            </div>
...
const previewUrl = ref(resolveImagePath(props.brand?.image_path));
...
const handleImageChange = (e: Event) => {
  ...
  if (file) {
    form.image = file;
    previewUrl.value = URL.createObjectURL(file); // Local preview
  }
};

function submit() {
  form.post(`/dashboard/brands/update/${props.brand?.slug}`, {
    forceFormData: true,
  });
}
</script>
